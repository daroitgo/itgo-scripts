...
# ========= TSEQ (Tomcat Sequencer) Installer v3.11 =========
VERSION="3.11"
BASE_USER="itgo"
...
interactive_configure_order() {
  ...
  if ! prompt_yn "TSEQ: Ustawić teraz kolejność i delay w order.txt (interaktywnie)?" "Y"; then
  ...
  tmp="$(mktemp)"
  {
    echo "# Kolejność: mniejszy numer = wcześniej"
    echo "# Format: <NUMER> <USŁUGA> [POST_DELAY_S]"
    echo "# lub: - <USŁUGA> (wyłączone)   lub # komentarz"
    echo "# POST_DELAY_S = ile sekund czekać po markerze READY zanim odpali następną usługę"
    echo
  } >"$tmp"

  local i n_default num delay
  n_default=10
  ...
  for i in "${!svcs[@]}"; do
    ...
    if prompt_yn "Uwzględnić tę usługę w order.txt?" "Y"; then
      ...
      delay="$(prompt_int_default "POST_DELAY_S (sekundy po READY)" "0")"
      printf "%s %s %s\n" "$num" "$svc" "$delay" >>"$tmp"
...
while read -r line; do
  ...
  if [[ "\$first" =~ ^[0-9]+\$ ]]; then
    ...
    if [[ -n "\${MAP[\$svc]:-}" ]]; then
      ...
      log "ORDER: listed but not mapped (ignored): \$svc"
    fi
  else
...
write_default_order_if_missing() {
  ...
  local n=10
  {
    ...
    echo "# POST_DELAY_S = ile sekund czekać po markerze READY zanim odpali następną usługę"
    echo
    awk -F"\t" '$1 !~ /^#/ && $1 != "" {print $1}' "$DISCOVERED_TSV" | while read -r svc; do
      [[ -n "${svc:-}" ]] || continue
      printf "%d %s\n" "$n" "$svc"
      n=$((n+10))
    done
